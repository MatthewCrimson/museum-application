...
$('#submitbutton3').click(function () {
  ...
  {
    var old_values = [];
    firebase.database().ref("Votes/Gallery3/item" + selected_list[i]).once('value', function(snapshot){
      ...
      if (old_values.length == selected_list.length){
        ...
        for(z = 0; z < selected_list.length; z++){
          firebase.database().ref("Votes/Gallery3/item" + selected_list[z]).set(old_values[z] + 1);
        }
